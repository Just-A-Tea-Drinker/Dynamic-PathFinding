...
void Movement::Obs_avoid()
{
    ...
    if(sqrt((EndPoint[0]-X)*(EndPoint[0]-X) +((EndPoint[1]-Y)*(EndPoint[1]-Y)))<=0.05)
    {
        ROS_INFO("Target Found");
        Lin_Vel = 0.0f;
        Ang_Vel = 0.0f;
    }
    else if(sqrt((WayPoint[0]-X)*(WayPoint[0]-X) +((WayPoint[1]-Y)*(WayPoint[1]-Y)))<=0.05)
    {
